...
if (isset($_POST['opt_update'])) {
    ...
    foreach ($_POST['option'] as $key => $val) {
        ...
        try {
            ...
            $stmt = $db->prepare("UPDATE sp_options SET value = :value WHERE name = :name");
            $stmt->bindValue(':value', $option, PDO::PARAM_STR);
            $stmt->bindValue(':name', $option_name, PDO::PARAM_STR);
            $result = $stmt->execute();
    ...
    // هدایت به صفحه مناسب بر اساس نتیجه
    if ($success && !$error) {
        if (isset($_GET['page'])) { ?>
