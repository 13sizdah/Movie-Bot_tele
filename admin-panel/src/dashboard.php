...
// اطمینان از اینکه func.php include شده است
// func.php باید در head.php include شده باشد
// اما برای اطمینان، اگر تابع وجود ندارد، دوباره include می‌کنیم
if (!function_exists('total_income')) {
    // اگر func.php include نشده، آن را include کن
    $func_path = __DIR__ . DIRECTORY_SEPARATOR . 'func.php';
    if (file_exists($func_path)) {
        include_once $func_path;
    } else {
        // آخرین تلاش با مسیر نسبی
        $func_path_alt = __DIR__ . '/func.php';
        if (file_exists($func_path_alt)) {
            include_once $func_path_alt;
        } else {
            die('❌ فایل func.php یافت نشد. تابع total_income تعریف نشده است. مسیرهای بررسی شده: ' . htmlspecialchars($func_path) . ' و ' . htmlspecialchars($func_path_alt));
        }
    }
}

// اگر هنوز تابع وجود ندارد، یک تابع پیش‌فرض تعریف می‌کنیم
if (!function_exists('total_income')) {
    function total_income() {
        ...
        try {
            $sql = "SELECT SUM(price) as total FROM sp_orders WHERE status=1";
            ...
            <div class="flex flex-wrap -m-4 text-center">
                ...
                <div class="p-4 md:w-1/3 sm:w-1/2 w-full">
                    <div class="bg-yellow-400 px-4 py-6 rounded-lg">
                        ...
                        <p class="leading-relaxed text-white">تعداد فیلم و سریال</p>
                    </div>
                </div>
