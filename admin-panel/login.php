...
if (file_exists($db_path)) {
    ...
    die('❌ فایل db.php یافت نشد. مسیر: ' . htmlspecialchars($db_path));
}
...
?>
    ...
    <div class="flex flex-col w-full max-w-md px-4 py-8 bg-white rounded-lg shadow dark:bg-gray-800 sm:px-6 md:px-8 lg:px-10">
        ...
        if (isset($_GET['error'])) {
            ...
            switch ($_GET['error']) {
                case 'no_token':
                    $error_msg = 'توکن احراز هویت ارسال نشده است.';
                    break;
                case 'invalid_token':
                    $error_msg = 'توکن نامعتبر است.';
                    break;
                case 'expired_token':
                    $error_msg = 'توکن منقضی شده است. لطفاً دوباره از ربات درخواست کنید.';
                    break;
                case 'admin_not_found':
                    $error_msg = 'ادمین یافت نشد.';
                    break;
                default:
                    $error_msg = 'خطا در احراز هویت.';
            }
            echo '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4" role="alert">' . htmlspecialchars($error_msg) . '</div>';
        }
        ?>
        <div class="mt-8">
...
function login_user($username, $password)
{
    ...
    // بررسی وجود $db
    if (!isset($db)) {
        echo "<span class='bg-red-600 text-white text-xl p-2 m-2 rounded-lg'>خطا در اتصال به دیتابیس!</span>";
        return false;
    }
